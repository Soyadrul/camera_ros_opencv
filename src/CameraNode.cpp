...
#include <cctype>
#include <cerrno>
#include <chrono> // Aggiunte
#include <cstdint>
#include <cstring>
#include <ctime> // Aggiunte
#include <cv_bridge/cv_bridge.hpp> // Aggiunte
...
#endif
#include <atomic>
#include <filesystem> // Aggiunte
#include <iomanip> // Aggiunte
#include <iostream>
...
private:
  ...
  std::string save_directory_; // Aggiunte
  bool save_images_; // Aggiunte
      ...
      )
{
  ...
  this->declare_parameter("save_images", false); // Aggiunte
  this->declare_parameter("save_directory", "/home/pi/camera_images"); // Aggiunte
  
  save_images_ = this->get_parameter("save_images").as_bool(); // Aggiunte
  save_directory_ = this->get_parameter("save_directory").as_string(); // Aggiunte
  
  // Aggiunte (inizio)
  // Crea la directory se non esiste e save_images è true
    if (save_images_) {
    ...
    // Esegui il comando di sistema richiesto
    int s1 = std::system("python3 /home/pi/ros2_ws/src/wasr_deploy/predict_single.py --weight /home/pi/ros2_ws/src/wasr_deploy/weights/wasr_rn101.pth /home/pi/camera_images/2.jpg /home/pi/ros2_ws/src/wasr_deploy/output/predictions/2.jpg");
    RCLCPP_INFO(this->get_logger(), "System command returned: %d", s1);
    // Aggiunte (fine)
}
...
CameraNode::process(libcamera::Request *const request)
{
  while (true) {
    ...
    if (request->status() == libcamera::Request::RequestComplete) {
      ...
      // Aggiunte (inizio)
      // Se l'opzione di salvataggio è attivata, salva l'immagine
      if (save_images_) {
        try {
            // Converti da messaggio ROS a formato OpenCV
            std::shared_ptr<sensor_msgs::msg::Image> shared_msg_img = std::move(msg_img);
            std::shared_ptr<const sensor_msgs::msg::Image> const_shared_msg = shared_msg_img;
            cv_bridge::CvImageConstPtr cv_ptr = cv_bridge::toCvShare(shared_msg_img, "bgr8");
            
            // Genera un nome file basato sul timestamp
            //auto now = std::chrono::system_clock::now();
            //auto in_time_t = std::chrono::system_clock::to_time_t(now);
            std::stringstream ss;
            /*ss << save_directory_ << "/image_" 
               << std::put_time(std::localtime(&in_time_t), "%Y%m%d_%H%M%S")
               << "_" << std::chrono::duration_cast<std::chrono::milliseconds>(
                     now.time_since_epoch()).count() % 1000
               << ".jpg";*/
            ss << save_directory_ << "/2" << ".jpg";
            
            // Salva l'immagine
            cv::imwrite(ss.str(), cv_ptr->image);
            RCLCPP_INFO_THROTTLE(this->get_logger(), 
                               *this->get_clock(), 
                               5000, // Log ogni 5 secondi
                               "Saved image to: %s", ss.str().c_str());
        } catch (const cv_bridge::Exception& e) {
